...
export const importClientsFromCSV = async (file: File): Promise<{ 
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Correctly map client fields to database columns for Supabase
      clients.push({
        nom: client.nom,
        prenom: client.prenom,
        email: client.email,
    ...
    // Insert clients in batch, using the correct field structure
    // Note: We're not providing the 'id' field, as it will be auto-generated by Supabase
    const { error } = await supabase
      .from('profiles')
      .insert(clients);
