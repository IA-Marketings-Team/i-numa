...
const SuperviseurEquipe = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const teamsCollection = realmUser.mongoClient("mongodb-atlas").db("inuma").collection("teams");
      const teamsResult = await teamsCollection.find({});
      const teamsData = teamsResult.map((team: any) => ({
        id: team._id.toString(),
        nom: team.nom,
        fonction: team.fonction,
        description: team.description || '',
        dateCreation: new Date(team.dateCreation)
      }));
      setTeams(teamsData);
      
      const agentsCollection = realmUser.mongoClient("mongodb-atlas").db("inuma").collection("users");
      const agentsResult = await agentsCollection.find({
        role: { $in: ['agent_phoner', 'agent_visio', 'agent_developpeur', 'agent_marketing'] }
      });
      const agentsData = agentsResult.map((agent: any) => ({
        ...
        statistiques: agent.statistiques || {
          appelsEmis: 0,
          appelsDecroches: 0,
          appelsTransformes: 0,
          rendezVousHonores: 0,
      ...
      const tasksCollection = realmUser.mongoClient("mongodb-atlas").db("inuma").collection("tasks");
      const tasksResult = await tasksCollection.find({});
      const tasksData = tasksResult.map((task: any) => ({
        id: task._id.toString(),
        title: task.title,
        description: task.description || '',
        agentId: task.agentId,
        status: task.status,
        dateCreation: new Date(task.dateCreation),
